...
//	
//	public SecurityConfig(JWTFilter filter, JWTProvider provider, SecurityService service) {
//		this.filter = filter;
...
//			String token = provider.inssuanceToken(user_id, auth.getAuthorities().stream().map(arg-> arg.getAuthority()).collect(Collectors.toList()));
